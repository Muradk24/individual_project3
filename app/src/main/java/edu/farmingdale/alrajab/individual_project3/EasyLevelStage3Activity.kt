...
class EasyLevelStage3Activity : AppCompatActivity() {

    private var mediaPlayer: MediaPlayer? = null
    lateinit var binding: ActivityEasyLevelStage3Binding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityEasyLevelStage3Binding.inflate(layoutInflater)
...
// after 1 click on loading the fucntion you need to click it again to play the anamation
    ...
    private fun animSnowman() {
        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationY", 1250f).apply {
            duration = 2000
            start()
            addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    if (binding.holder02.tag == 3) {
                        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationX", 1050f)
                            .apply {
                                duration = 2000
                                start()
                                addListener(object : AnimatorListenerAdapter() {
