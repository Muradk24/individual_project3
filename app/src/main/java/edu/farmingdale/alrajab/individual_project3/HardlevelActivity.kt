...
import edu.farmingdale.alrajab.individual_project3.databinding.ActivityEasyLevelStage3Binding
import edu.farmingdale.alrajab.individual_project3.databinding.ActivityHardlevelBinding
import java.io.IOException

class HardlevelActivity : AppCompatActivity() {
    lateinit var binding: ActivityHardlevelBinding

    private var mediaPlayer: MediaPlayer? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityHardlevelBinding.inflate(layoutInflater)
...
// after 1 click on loading the fucntion you need to click it again to play the anamation
    ...
    private fun animSnowman() {
        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationX", 600f).apply {
            duration = 2000
            start()
            addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    if (binding.holder02.tag == 2) {
                        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationY", 850f)
                            .apply {
                                duration = 2000
                                start()
                                addListener(object : AnimatorListenerAdapter() {
                                    override fun onAnimationEnd(animation: Animator) {
                                        if (binding.holder03.tag == 3) {
    ...
    private fun success() {
        ...
        retrying?.setOnClickListener {
            builder.dismiss()
            startActivity(Intent(this, HardActivityStage2::class.java))
            finish()
        }
