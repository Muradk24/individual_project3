...
import edu.farmingdale.alrajab.individual_project3.databinding.ActivityHardStage2Binding
import edu.farmingdale.alrajab.individual_project3.databinding.ActivityHardStage3Binding
import java.io.IOException

class HardActivityStage3 : AppCompatActivity() {
    lateinit var binding: ActivityHardStage3Binding

    private var mediaPlayer: MediaPlayer? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityHardStage3Binding.inflate(layoutInflater)
...
// after 1 click on loading the fucntion you need to click it again to play the anamation
    ...
    private fun animSnowman() {
        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationX", 1150f).apply {
            duration = 2000
            start()
            ...
            addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    if (binding.holder02.tag == 2) {
                        ObjectAnimator.ofFloat(binding.imageViewsnowman, "translationY", 550f)
                            .apply {
                                duration = 2000
                                start()
                                addListener(object : AnimatorListenerAdapter() {
                                    override fun onAnimationEnd(animation: Animator) {
                                        if (binding.holder03.tag ==4) {

                                            ObjectAnimator.ofFloat(
                                                binding.imageViewsnowman,
                                                "translationX", -1f
                                            ).apply {
                                                duration = 2000
                                                start()
            ...
            when (dragEvent.action) {
                ...
                DragEvent.ACTION_DROP -> {
                    ...
                    val item: ClipData.Item =
                        dragEvent.clipData.getItemAt(0)
    ...
    private class ArrowDragShadowBuilder(view: View) :
